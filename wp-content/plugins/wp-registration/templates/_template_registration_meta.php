...
echo "<input type='hidden' name='_post_id' id='_post_id' value='". (isset($_REQUEST['postid'])?$_REQUEST['postid']:'')."'>";
		...
		foreach($existing_meta as $key => $meta)
		{
			...
			switch($type)
			{
				...
				case 'text':
					...
					echo '<p id="box-'.$name.'" '.$conditions_data.'>';
				...
				case 'textarea':
					...
					echo '<div id="box-'.$name.'" '.$conditions_data.'>';
